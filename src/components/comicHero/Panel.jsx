// src/components/comicHero/Panel.jsx
// Chuyển đổi từ infinite-heroes/Panel.tsx (bỏ kiểu TypeScript).
        ...
        >
          ...
          {face.choices.map((choice, i) => (
            <button
              ...
              className={`comic-btn w-full py-3 text-xl font-bold tracking-wider ${
                i === 0 ? 'bg-yellow-400 hover:bg-yellow-300' : 'bg-blue-500 text-white hover:bg-blue-400'
              }`}
            >
              {choice}
            </button>
        ...
        <div className="absolute bottom-20 inset-x-0 flex justify-center z-20">
          ...
          >
            {!allFaces.find((f) => f.pageIndex === GATE_PAGE)?.imageUrl
              ? `PRINTING... ${allFaces.filter((f) => f.type === 'story' && f.imageUrl && (f.pageIndex || 0) <= GATE_PAGE).length}/${INITIAL_PAGES}`
              : 'READ ISSUE #1'}
          </button>
        </div>
      )}
